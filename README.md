...
Actually, this map was created for the parameter **p = 0**. But, if we change this parameter, then the map will shift relative to the diagonals (It can be shown only for small highlighted fragment, due to the long experiment):
...
For this task, I decided to take the old wavelength tuning with joints and remove points that deviate from the linear dependence. Then, I interpolated the resulting dependence. I've got the required parameters for the laser and this wavelength tuning:
